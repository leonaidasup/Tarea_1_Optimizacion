...
def false_position(f, xl: float, xu: float, xr: float, error: int) -> object:
    e: float = 10 ** - error
    ...
    while inc_i > e:
        (xl, xu) = comparator(f=f, xl=xl, xu=xu, xr=xu - f(xu) * (xu - xl) / (f(xu) - f(xl)))
        inc_i = abs(xl - xu)
        list_inc_e_xl.append(abs(xr - xl))
        list_inc_e_xu.append(abs(xr - xu))
        list_inc_i.append(inc_i)
    return graphics(name='(False Position)', label='|x(u) - x(l)|',
